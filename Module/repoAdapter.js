...
module.exports = {
    ...
    StoreGallery: function(galleryId, imageGallery){
        const gallery = imageGallery.filter(img => img.image.slice(0,5) === "data:");
        ...
        return repo.CreateEmptyGallery(gid.toString())
            ...
            .then(() => {
                gallery.forEach( (img,i) => img.image = `//${config.repo.host}:${config.repo.port}${config.repo.webRoot}/${galleryId}/${fids[i]}.${exts[i]}`)
            })
            .catch(error => {console.error(error); throw error;});
    },
    DeleteGallery: function(galleryId, fileId){
